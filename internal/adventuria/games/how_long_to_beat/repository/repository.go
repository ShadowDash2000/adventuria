...
func (r *Repository) GetByNamePartsAndYear(ctx context.Context, nameParts []string, year int) ([]*model.HowLongToBeat, error) {
	pb := pbtransaction.GetCtxTransactionOrApp(ctx, r.pb)

	var records []*core.Record
	err := pb.RecordQuery(schema.CollectionHowLongToBeat).
		WithContext(ctx).
		Where(dbx.Like(schema.HowLongToBeatSchema.Name, nameParts...)).
		AndWhere(dbx.Or(
	...
	return RecordsToHowLongToBeat(records), nil
}
