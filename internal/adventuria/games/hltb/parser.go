package hltb

import (
	"adventuria/internal/adventuria/games/github"
	"context"
	"encoding/json"
	"net/http"

	"github.com/ShadowDash2000/howlongtobeat"
)

type Parser struct{}

func NewParser() *Parser {
	return &Parser{}
}

func (p *Parser) FetchLatestRelease(ctx context.Context) ([]howlongtobeat.SearchGameData, error) {
	downloadUrl, err := github.FetchLatestReleaseDownloadUrl(ctx, "ShadowDash2000", "hltb-scraper")
	if err != nil {
		return nil, err
	}

	client := http.DefaultClient
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, downloadUrl, nil)
	if err != nil {
		return nil, err
	}
	fileRes, err := client.Do(req)
	if err != nil {
		return nil, err
	}
	defer fileRes.Body.Close()

	var games struct {
		Data []howlongtobeat.SearchGameData `json:"data"`
	}
	if err = json.NewDecoder(fileRes.Body).Decode(&games); err != nil {
		return nil, err
	}

	return games.Data, nil
}
