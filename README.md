...
Примеры готовых игровых клеток: `internal/adventuria/cells`. В базовом варианте клетка должна имплементировать
только один метод: `OnCellReached(user adventuria.User) error`, который описывает то, что должно происходить
